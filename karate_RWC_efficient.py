...
def getRandomNodesFromLabels(G, k,
                             flag):  # parameter k = no. of random nodes to generate, flag could be "left", "right" or "both". If both, k/2 from one side and k/2 from the other side are generated.
    random_nodes = []
    random_nodes1 = {}
    if (flag == "left"):
        for i in range(k):
...
def getNodesFromLabelsWithHighestDegree(G, k,
                                        flag):  # first take the nodes with the highest degree according to the "flag" and then take the top $k$
    random_nodes = {}
    dict_degrees = {}
    for node in G.nodes():
...
def performRandomWalk(G, starting_node, user_nodes_side1,
                      user_nodes_side2):  # returns if we ended up in a "left" node or a "right" node
...
def performRandomWalkFull(G, starting_node,
                          user_nodes):  # returns the number of steps taken before reaching *ALL* node from the set of user nodes. difference from the above method is that we should reach all nodes, instead of just any one of them.
    dict_nodes = {}  # contains unique nodes seen till now
    nodes = G.nodes()
    num_edges = len(G.edges())
    step_count = 0
    ...
    while (flag != 1):
        ...
        if (user_nodes.has_key(starting_node)):
            ...
            print("seen nodes ", len(dict_already_seen_nodes.keys()))
...
for j in range(1, 1000):
    ...
    user_nodes_left = getNodesFromLabelsWithHighestDegree(G,1,"left")
    user_nodes_right = getNodesFromLabelsWithHighestDegree(G,1,"right")
    ...
    for i in range(len(user_nodes_left_list)):
    ...
    for i in range(len(user_nodes_right_list)):
        #		node = getRandomNodes(G,1).keys()[0]
